// Optimized Mobile Painter - hifive.games
...
namespace hifive_MobilePaint
{
    ...
    public class MobilePaint : MonoBehaviour
    {
        ...
        [Header("Mouse or Touch")] public bool enableTouch = false;

        [Space(10)] public LayerMask paintLayerMask = 1 << 0; // to which layer our paint canvas is at (used in raycast)
        ...
        public bool
            connectBrushStokes =
                false; // if brush moves too fast, then connect them with line. NOTE! Disable this if you are painting to custom mesh
        ...
        public bool
            useAdditiveColors =
                false; // true = alpha adds up slowly, false = 1 click will instantly set alpha to brush or paint color alpha value

        public float
            brushAlphaStrength = 0.01f; // multiplier to soften brush additive alpha, 0.1f is nice & smooth, 1 = faster
        ...
        // ERASER
        [Space(10)] public EraserMode eraserMode = EraserMode.BackgroundColor;
        ...
        [Header("Line Drawing")] public Transform previewLineCircle;
        ...
        [HideInInspector] public bool textureNeedsUpdate = false; // if we have modified texture

        [Header("Misc")] public bool undoEnabled = false;
        ...
        void Awake()
        {
            ...
            if (go == null)
            {
                Debug.LogError("GameObject EventSystem is missing from scene, will have problems with the UI",
                    gameObject);
            }
            else
            {
        ...
        {
            if (cam == null)
                Debug.LogError("MainCamera not founded, you must have 1 camera active & tagged as MainCamera",
                    gameObject);
            ...
            if (userInterface == null)
            {
                ...
                {
                    Debug.LogWarning("UI Canvas / userInterface not assigned - disabling hideUIWhilePainting",
                        gameObject);
            ...
            {
                Debug.LogWarning("paintLayerMask is set to 'nothing', assign same layer where the drawing canvas is",
                    gameObject);
            }
            ...
            if (overrideResolution)
            {
                if (overrideWidth < 0 || overrideWidth > 8192)
                {
                    Debug.LogWarning("Invalid overrideWidth:" + overrideWidth, gameObject);
                    overrideWidth = Mathf.Clamp(0, 1, 8192);
                }

                if (overrideHeight < 0 || overrideHeight > 8192)
                {
                    Debug.LogWarning("Invalid overrideHeight:" + overrideHeight, gameObject);
                    overrideHeight = Mathf.Clamp(0, 1, 8192);
                }

                if (resolutionScaler != 1)
                {
                    Debug.LogWarning(
                        "Cannot use resolutionScaler with OverrideResolution, setting resolutionScaler to default (1)");
                    resolutionScaler = 1;
                }
            }
            ...
            if (myRenderer.material.GetTexture(targetTexture) == null)
            {
                ...
                {
                    Debug.LogError("eraserMode is set to Default, but there is no texture assigned to " +
                                   targetTexture + ". Setting eraseMode to BackgroundColor");
        ...
        public void InitializeEverything()
        {
            ...
            else
            {
                ...
                if (connectBrushStokes)
                    Debug.LogWarning(
                        "Custom mesh used, but connectBrushStokes is enabled, it can cause problems on the mesh borders wrapping");
            ...
            ) // temporary fix by adding && !usingClearingImage
            {
                // create new texture
            ...
            else
            {
                ...
                if (overrideResolution)
                    Debug.LogWarning(
                        "overrideResolution is not used, when canvas texture is assiged to material, we need to use the texture size");
        ...
        void MousePaint()
        {
            ...
            if (Input.GetMouseButton(0))
            {
                ...
                //Physics.Raycast(drawRay.position, Vector3.down, out hit, 1f, paintLayerMask)
                if (!Physics.Raycast(drawRay.position, Vector3.down, out hit, 1f, paintLayerMask))
                {
                    wentOutside = true;
                    return;
                }
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (!Physics.Raycast(drawRay.position, Vector3.down, out hit, 1f, paintLayerMask)) return;
            ...
            if (Input.GetMouseButtonUp(0))
            {
                ...
                if (drawMode == DrawMode.ShapeLines)
                {
                    ...
                    {
                        Vector2 extendLine =
                            (pixelUV - new Vector2((float) firstClickX, (float) firstClickY)).normalized *
                            (brushSize * 0.25f);
                        DrawLine(firstClickX - (int) extendLine.x, firstClickY - (int) extendLine.y,
                            SnapToGrid((int) pixelUV.x + (int) extendLine.x),
                            SnapToGrid((int) pixelUV.y + (int) extendLine.y));
                    ...
                    else
                    {
                        ...
                        Vector2 extendLine =
                            ...
                            (brushSize * 0.25f);
                        DrawLine(firstClickX - (int) extendLine.x, firstClickY - (int) extendLine.y,
                            (int) pixelUV.x + (int) extendLine.x, (int) pixelUV.y + (int) extendLine.y);
                    }
        ...
        void TouchPaint()
        {
                    ...
                    if (Physics.Raycast(drawRay.position, Vector3.down, out hit, 1f, paintLayerMask))
                    {
                        ...
                        switch (drawMode)
                        {
                            ...
                            case DrawMode.ShapeLines:
                                if (snapLinesToGrid)
                                {
                                    DrawShapeLinePreview(SnapToGrid((int) pixelUVs[touch.fingerId].x),
                                        SnapToGrid((int) pixelUVs[touch.fingerId].y));
                                }
                                else
                                {
                                    DrawShapeLinePreview((int) pixelUVs[touch.fingerId].x,
                                        (int) pixelUVs[touch.fingerId].y);
                                }
                            ...
                            case DrawMode.Eraser:
                                ...
                                {
                                    EraseWithBackgroundColor((int) pixelUVs[touch.fingerId].x,
                                        (int) pixelUVs[touch.fingerId].y);
                                }
            ...
            if (wasTouching && Input.touchCount == 0)
            {
                ...
                if (drawMode == DrawMode.ShapeLines)
                {
                    ...
                    if (snapLinesToGrid)
                    {
                        DrawLine(new Vector2(firstClickX, firstClickY),
                            new Vector2(SnapToGrid((int) pixelUVs[touch.fingerId].x),
                                SnapToGrid((int) pixelUVs[touch.fingerId].y)));
                    }
                    else
                    {
        ...
        public void DrawPatternCircle(int x, int y)
        {
            ...
            if (createCanvasMesh
            ) // TEMPORARY FIX: with b custom sphere mesh, small gap in paint at the end, so must disable clamp on most custom meshes
            {
            ...
            for (int y = 0; y < customBrushHeight; y++)
            {
                for (int x = 0; x < customBrushWidth; x++)
                {
                    ...
                    if (customBrushBytes[brushPixel + 3] > 0 &&
                        (!useLockArea || (useLockArea && lockMaskPixels[pixel] == 1)))
                    {
                        if (useCustomBrushAlpha) // use alpha from brush
                        {
                            ...
                            {
                                brushAlphaLerpVal =
                                    customBrushBytes[brushPixel + 3] * brushAlphaStrength *
                                    0.01f; // 0.01 is temporary fix so that default brush & custom brush both work
                                ...
                                else
                                {
                                    ...
                                    pixels[pixel] = ByteLerp(pixels[pixel], customBrushBytes[brushPixel],
                                        brushAlphaLerpVal);
                                    pixels[pixel + 1] = ByteLerp(pixels[pixel + 1], customBrushBytes[brushPixel + 1],
                                        brushAlphaLerpVal);
                                    pixels[pixel + 2] = ByteLerp(pixels[pixel + 2], customBrushBytes[brushPixel + 2],
                                        brushAlphaLerpVal);
                                }
        ...
        void FloodFill(int x, int y)
        {
            ...
            // early exit if its same color already
            if (paintColor.r == hitColorR && paintColor.g == hitColorG && paintColor.b == hitColorB &&
                paintColor.a == hitColorA) return;
        ...
        void LockMaskFillWithThreshold(int x, int y)
        {
            ...
            while (fillPointX.Count > 0)
            {
                ...
                if (ptsy - 1 > -1)
                {
                    ...
                    if (lockMaskPixels[pixel] == 0 // this pixel is not used yet
                        && (CompareThreshold(pixels[pixel + 0], hitColorR) ||
                            CompareThreshold(pixels[pixel + 0], paintColor.r)
                        ) // if pixel is same as hit color OR same as paint color
                        && (CompareThreshold(pixels[pixel + 1], hitColorG) ||
        ...
        public void DrawLine(int startX, int startY, int endX, int endY)
        {
            ...
            for (;;) // endless loop
            {
                ...
                else
                {
                    ...
                    ) // might have small gaps if this is used, but its alot(tm) faster to skip few pixels
                    {
                        pixelCount = 0;
                        DrawCircle(startX, startY);
                    }
		...
		*/
        ...
        public void DoUndo()
        {
            if (undoEnabled)
            {
                ...
                {
                    System.Array.Copy(undoPixels[undoPixels.Count - 1], pixels,
                        undoPixels[undoPixels.Count - 1].Length);
        ...
        void CreateFullScreenQuad()
        {
            ...
            if (referenceArea) // use reference object & canvas scaling for size
            {
                ...
                if (temporaryCanvasArray == null || temporaryCanvasArray.Length == 0)
                    Debug.LogError("Canvas not found from ReferenceArea parent", gameObject);
                if (temporaryCanvasArray.Length > 1)
                    Debug.LogError("More than 1 Canvas was found from ReferenceArea parent, can cause problems",
                        gameObject);
                ...
                referenceCorners[0] = new Vector3(referenceArea.offsetMin.x * canvasScaleFactor,
                    referenceArea.offsetMin.y * canvasScaleFactor, 0);
                referenceCorners[1] = new Vector3(referenceArea.offsetMin.x * canvasScaleFactor,
                    Screen.height + referenceArea.offsetMax.y * canvasScaleFactor, 0);
                referenceCorners[2] = new Vector3(Screen.width + referenceArea.offsetMax.x * canvasScaleFactor,
                    Screen.height + referenceArea.offsetMax.y * canvasScaleFactor, 0);
                referenceCorners[3] = new Vector3(Screen.width + referenceArea.offsetMax.x * canvasScaleFactor,
                    referenceArea.offsetMin.y * canvasScaleFactor, 0);
            ...
            else
            {
                ...
                referenceCorners[1] =
                    new Vector3(0, cam.pixelHeight + canvasSizeAdjust.y, cam.nearClipPlane); // top left
                referenceCorners[2] = new Vector3(cam.pixelWidth + canvasSizeAdjust.x,
                    cam.pixelHeight + canvasSizeAdjust.y, cam.nearClipPlane); // top right
                referenceCorners[3] = new Vector3(cam.pixelWidth + canvasSizeAdjust.x, canvasSizeAdjust.y,
                    cam.nearClipPlane); // bottom right
            }
        ...
        {
            var image = new Texture2D((int) (texWidth / resolutionScaler), (int) (texHeight / resolutionScaler),
        ...
        public Texture2D GetScreenshot()
        {
            ...
            var topRight = cam.WorldToScreenPoint(go_Mesh.vertices[3]);
            var bottomRight = cam.WorldToScreenPoint(go_Mesh.vertices[2]);
            var image = new Texture2D((int) (bottomRight.x - topLeft.x), (int) (bottomRight.y - topRight.y),
...
#pragma warning restore 618
        ...
        public void SetPanZoomMode(bool state)
        {
            ...
            this.enabled =
                isZoomingOrPanning ? false : true; // Disable Update() loop from this script, if zooming or panning
        }


        // cleaning up buffers - https://github.com/hifive/UnityMobilePaint/issues/10
        void OnDestroy()
        {
