...
COMPANY='Hifive Games'
GAME_NAME='new_game'
BUNDLE='com.hifive.game'
TEAM='MYQT253339'
...
SUPPORT_FILES_PATH=$PROJECT_PATH'/SupportFiles'
FASTLANE_FOLDER_PATH=$IOS_PATH'/fastlane'
FASTFILE_TEMPLATE=$SUPPORT_FILES_PATH'/Fastfile'
...
UNITY='/Applications/Unity/Hub/Editor/2019.4.15f1/Unity.app/Contents/MacOS/Unity'
...
echo '' 
$UNITY -batchmode -quit -projectPath "$PROJECT_PATH" -executeMethod Hifive.ProjectBuilder.BuildActions.AndroidDevelopment -buildTarget android -logFile "$LOGS_PATH/android_development.log"
if [ $? -ne 0 ]; then
echo ''
...
echo 'build unity and archive APK...' 
echo '' 
$UNITY -batchmode -quit -projectPath "$PROJECT_PATH" -executeMethod Hifive.ProjectBuilder.BuildActions.AndroidRelease -buildTarget android -logFile "$LOGS_PATH/android_release.log"
if [ $? -ne 0 ]; then
echo ''
echo 'Operation failed!'
echo '' 
...
echo 'build unity...' 
echo '' 
$UNITY -batchmode -quit -projectPath "$PROJECT_PATH" -executeMethod Hifive.ProjectBuilder.BuildActions.iOSDevelopment -buildTarget ios -logFile "$LOGS_PATH/ios_development.log"
if [ $? -ne 0 ]; then
echo ''
echo 'Operation failed!'
echo '' 
...
echo 'build unity...' 
echo '' 
$UNITY -batchmode -quit -projectPath "$PROJECT_PATH" -executeMethod Hifive.ProjectBuilder.BuildActions.iOSRelease -buildTarget ios -logFile "$LOGS_PATH/ios_release.log"
if [ $? -ne 0 ]; then
echo ''
echo 'Operation failed!'
echo '' 
...
echo 'build completed' 
echo ''
echo 'initialize fastlane...'
cd $IOS_PATH
sudo fastlane init
echo ''
echo 'remove default Fastfile from fastlane folder in XCode project root...'
echo ''
echo 'copy Fastfile to fastlane folder in XCode project root...'
sudo mv "Fastfile" "$FASTLANE_FOLDER_PATH"
echo ''
echo 'call fastlane beta...'
sudo fastlane beta
}
